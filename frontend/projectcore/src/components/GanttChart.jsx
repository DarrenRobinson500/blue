...
export default function GanttChart({ tasks: propTasks, onSave, onDependencyCreate, onDeleteLink, onTaskEdit, selectedTaskId, onTaskSelect, onTaskDone, isMultiProject, onAddTask }) {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (drag.type === 'link') {
      ...
      const hov = idx >= 0 && idx < tasks.length && !tasks[idx]?.isHeader && !tasks[idx]?.is_heading ? tasks[idx].id : null
  ...
  const handleMouseUp = useCallback((e) => {
    ...
    if (drag.type === 'link') {
      ...
      if (idx >= 0 && idx < tasks.length) {
        ...
        if (target && !target.isHeader && !target.is_heading && target.id !== drag.fromId) {
          onDependencyCreate(target.id, drag.fromId)
        }
      }
  ...
  const arrows = localTasks
    .filter(t => t.depends_on != null && !t.isHeader && !t.is_heading)
    .flatMap(t => {
      ...
      <div style={{ width: px(SIDEBAR_W + TOTAL_DAYS * DAY_W), minWidth: '100%' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {localTasks.map((task, idx) => {
            ...
            // Section heading row (is_heading task)
            if (task.is_heading) {
              return (
                <div key={task.id} style={{ display: 'flex', height: px(ROW_H), borderBottom: '1px solid #e5e7eb' }}>
                  <div
                    style={{ width: px(SIDEBAR_W), flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6, padding: '0 10px', borderRight: '1px solid #e5e7eb', position: 'sticky', left: 0, background: '#fafafa', zIndex: 10 }}
                  >
                    <span
                    ...
                    >⠿</span>
                    <span
                      style={{ fontSize: 11, fontWeight: 700, color: '#6b7280', flex: 1, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', textTransform: 'uppercase', letterSpacing: '0.05em' }}
                      onDoubleClick={(e) => { e.stopPropagation(); onTaskEdit(task) }}
                    >{task.name}</span>
                  </div>
                  <div style={{ width: px(TOTAL_DAYS * DAY_W), background: '#fafafa', flexShrink: 0 }} />
                </div>
              )
            }
          ...
          {localTasks.filter(t => !t.isHeader && !t.is_heading).length === 0 && (
            <div style={{ padding: '48px 24px', textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>
              No tasks yet — click <strong>+ Task</strong> to get started.
            </div>
          )}
        </div>
      </div>
