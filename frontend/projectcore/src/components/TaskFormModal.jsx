...
export default function TaskFormModal({ task, projectId, defaultStartDate, onSuccess, onClose }) {
  ...
  const computedEndDate = form.is_heading
    ? form.start_date
    : format(addDays(parseISO(form.start_date), Math.max(1, Number(form.duration)) - 1), 'yyyy-MM-dd')

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.is_heading && Number(form.duration) < 1) {
    ...
    const payload = {
      ...
      start_date: form.is_heading ? null : form.start_date,
      end_date: form.is_heading ? null : computedEndDate,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => setForm(f => ({ ...f, is_heading: e.target.checked }))}
              className="w-4 h-4 rounded accent-indigo-600"
            />
            <span className="text-sm text-gray-700">Section heading (no dates)</span>
          </label>
          {!form.is_heading && (
            <>
              <div>
